...
const sortStarWarsCharacters = (starWarsArr) => {
  let heightArr = [];
  heightArr = starWarsArr.sort((a, b) => parseInt(b.height) - parseInt(a.height));
  return heightArr;
};
...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i <= strArr.length; i++) {
    let slicedArr = strArr.slice(i, strArr.length);
    let joinedStr = slicedArr.join('');
