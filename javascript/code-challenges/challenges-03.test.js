...
// DONE
// I used arr.filter in the group project for 201.
...
const typeNum = (arr) => {
  let newArr = arr.filter(isNumber);
  return newArr;
};
...
const containsAnd = (arr) => {
  let newArr = arr.filter(includesAnd);
  return newArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let notInBoth = arr.filter(val => !forbiddenValues.includes(val));
  return notInBoth;
};
