...
const addValues = (arr) => {
  const init = 0;
  const sum = arr.reduce((acc, cur) => acc + cur, init);
  return sum;
};
...
const addPurchases = (arr) => {
  const init = 0;
  const totalPurchase = arr.reduce((acc, cur) => acc + cur.purchasePrice, init);
...
const countNumberOfElements = (arr) => {
  const init = 0;
  const numElms = arr.reduce((acc) => acc + 1, init);
  return numElms;
};
...
const returnNames = (arr) => {
  const init = [];
  arr.reduce((acc, cur) => init.push(cur.name), 0);
  return init;
};
...
const reversedString = (str) => {
  ...
  console.log(split);
  const revStr = split.reduce((acc, cur) => cur + acc, '');
  return revStr;
};
